...
public class UsuarioServiceImp extends CrudGenericoServiceImp<Usuario, Long> implements IUsuarioService {
    ...
    // NUEVA CONFIGURACIÓN PARA PUNTOS
    ...
    public UsuarioDTO registerUser(UsuarioDTO.UsuarioCrearDto userDto) {
        ...
        String verificationLink = "http://localhost:8080/verificar/verify?token=" + vt.getToken();
        ...
        // *** NUEVA LÍNEA: OTORGAR PUNTOS POR REGISTRO ***
        try {
            puntoService.otorgarPuntos(
                    ...
                    puntosRegistro,
                    null,
                    "Puntos de bienvenida por registro de cuenta"
            );
        } catch (Exception e) {
            // Log del error pero no fallar el registro
            System.err.println("Error al otorgar puntos de registro: " + e.getMessage());
            e.printStackTrace();
        }

        try {
            preferenceService.createDefaultPreferences(savedUser.getIdUsuario());
            log.info("Preferencias por defecto creadas para el usuario ID: {}", savedUser.getIdUsuario());
        } catch (ExcepcionPersonalizada e) {
            log.error("Fallo crítico al crear preferencias por defecto para el usuario ID {}: {}", savedUser.getIdUsuario(), e.getMessage());
            // Esto anulará todo el registro del usuario (incluido el usuario, rol, token, puntos).
            throw new ExcepcionPersonalizada("Fallo al crear preferencias al registrar usuario: " + e.getMessage(), e);
        } catch (Exception e) { // Captura cualquier otra excepción inesperada
            log.error("Error inesperado al crear preferencias por defecto para el usuario ID {}: {}", savedUser.getIdUsuario(), e.getMessage(), e);
            // Relanzar como tu ExcepcionPersonalizada para mantener consistencia
            throw new ExcepcionPersonalizada("Error inesperado al crear preferencias al registrar usuario.", e);
        }
    ...
    public UsuarioDTO registerByAdmin(UsuarioDTO.UsuarioCrearConRolDto userDto) {
        ...
        try {
            preferenceService.createDefaultPreferences(savedUser.getIdUsuario());
            log.info("Preferencias por defecto creadas para el usuario ID: {} por ADMIN", savedUser.getIdUsuario());
        } catch (ExcepcionPersonalizada e) {
            log.error("Fallo crítico al crear preferencias por defecto para usuario ID {} (ADMIN): {}", savedUser.getIdUsuario(), e.getMessage());
            throw new ExcepcionPersonalizada("Fallo al crear preferencias para usuario registrado por ADMIN: " + e.getMessage(), e);
        } catch (Exception e) {
            log.error("Error inesperado al crear preferencias por defecto para usuario ID {} (ADMIN): {}", savedUser.getIdUsuario(), e.getMessage(), e);
            throw new ExcepcionPersonalizada("Error inesperado al crear preferencias para usuario registrado por ADMIN.", e);
        }
